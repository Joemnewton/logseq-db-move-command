...
async function moveBlockToPage(blockUuid: string, targetPageName: string) {
  try {
...
async function moveBlockToJournalDate(blockUuid: string, date: Date) {
  try {
    const block = await logseq.Editor.getBlock(blockUuid);
    if (!block) {
      logseq.UI.showMsg("Block not found", "error");
...
async function searchPages(query: string): Promise<Page[]> {
  try {
    const allPages = (await logseq.Editor.getAllPages()) as Page[];
...
async function showPageSearchDialog(blockUuid: string) {
  const html = `
...
async function showDatePickerDialog(blockUuid: string) {
